...
# the deltaP is the total pressure drop
    def calc_R_n(self, mdot, mu, roe, deltaP):
        R_n = 0.475*np.sqrt(mdot/(mu*np.sqrt(roe*deltaP)))
        return R_n
#step 3
    '''
...
# mdot, roe, and nu are user input values
    def calc_Re(self, mdot, n, r_in, roe, nu):
        self.Re = 0.637*mdot/(np.sqrt(n)*r_in*roe*nu)
#step 6
...
# R_in, R_n, n, r_in, Re are calculated from previous steps
    ...
    def calc_A_eq(self, R_in, R_n, n, r_in, lam):
        self.A_eq = R_in*R_n/(n*r_in^2+lam/2*R_in*(R_in-R_n))
    def calc_mu_eq(self): #NOTE: complete these quations
        mu_eq = 0
        return mu_eq
    def calc_alpha_eq(self):
        alpha_eq = 0
        return alpha_eq
#step 7
#Calculate the hydraulic-loss coefficient in the tangential passages
# first eps_in must be obtained
    def calc_eps_in(self): #NOTE: complete this quation
        eps_in = 0
        return eps_in
    def calc_eps(self, eps_in, lam, l_in, r_in):
        self.eps = eps_in + lam*l_in/(2*r_in)
#step 8
#Determine the actual flow coefficient
#NOTE: check what rbar_in is
    def calc_mu(self, mu_eq, eps, Rbar_in, A):
        self.mu = mu_eq/np.sqrt(1+eps*mu_eq^2*A/Rbar_in^2)
#step 9
# Calculate the nozzle radius using the new approximation
# this step reuses the function from step 2

#step 10
# Calculate the geometric parameter A in the new approximation
    def calc_A(self,R_in, R_n, n, r_in):
        self.A = R_in*R_n/(n*r_in^2)
